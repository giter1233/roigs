...
from utils.loss_utils import l1_loss, laplacian_pyramid_dynamic_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, debug_from, websockets, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # ROI-aware weighted L1 after a certain iteration
        use_roi_weights = (
            getattr(opt, 'use_mask', False)
            and hasattr(viewpoint_cam, 'mask')
            and viewpoint_cam.mask is not None
            and iteration >= getattr(opt, 'roi_loss_focus_after', 10**9)
        )
        if use_roi_weights:
            m = viewpoint_cam.mask
            # to (1,1,H,W) float on CUDA
            m2d = m
            if m2d.dim() == 3:
                m2d = m2d[0]
            m2d = m2d.float().to(gt_image.device)
            m2d = m2d.unsqueeze(0).unsqueeze(0)
            # dilation
            d = int(getattr(opt, 'roi_mask_dilate', 0))
            if d > 0:
                m2d = F.max_pool2d(m2d, kernel_size=2 * d + 1, stride=1, padding=d)
            # soft edges via blur
            if bool(getattr(opt, 'roi_mask_soft_edges', False)):
                k = int(getattr(opt, 'roi_mask_blur_kernel', 5))
                if k > 1 and k % 2 == 1:
                    m2d = F.avg_pool2d(m2d, kernel_size=k, stride=1, padding=k // 2)
            m2d = m2d.clamp(0.0, 1.0)
            outside_w = float(getattr(opt, 'roi_loss_outside_weight', 0.05))
            weight_map = outside_w + (1.0 - outside_w) * m2d  # (1,1,H,W)
            # expand to channels
            w3 = weight_map
            if gt_image.dim() == 3:
                w3 = weight_map.expand(1, gt_image.shape[0], gt_image.shape[1], gt_image.shape[2])
                pred_b = image.unsqueeze(0)
                gt_b = gt_image.unsqueeze(0)
                diff = torch.abs(pred_b - gt_b)
                Ll1 = (diff * w3).sum() / (w3.sum() + 1e-8)
            else:
                # Fallback
        ...
        # LPDR Loss with configurable parameters
        if opt.lpdr_weight > 0.0:
            lpdr = laplacian_pyramid_dynamic_loss(
                image, gt_image,
                levels=opt.lpdr_levels,
                kernel_size=opt.lpdr_kernel_size,
                sigma=opt.lpdr_sigma,
                weight_gamma=opt.lpdr_weight_gamma,
                include_dc=opt.lpdr_include_dc,
                dc_weight=opt.lpdr_dc_weight
            )
            loss = base_loss + opt.lpdr_weight * lpdr
        else:
            loss = base_loss
        ...
        with torch.no_grad():
            ...
            if False and iteration == 300:
                pass  # [no-prune] disabled per request
            ...
            if opt.densify_from_iter < iteration < opt.densify_until_iter:
                ...
                if iteration % opt.densification_interval == 0:
                    ...
                    current_mask = None
                    if getattr(opt, 'use_mask', False) and hasattr(viewpoint_cam, 'mask') and viewpoint_cam.mask is not None:
                        gate_mode = getattr(opt, 'roi_gate_mode', 'always')
                        if gate_mode == 'always' or iteration >= getattr(opt, 'roi_grow_from_iter', 0):
                            current_mask = viewpoint_cam.mask.cuda()
                    min_opacity_thr = getattr(opt, 'min_opacity_thr_when_mask', 0.0) if getattr(opt, 'use_mask', False) else 0.005
                    gaussians.densify_and_prune_Improved(gaussian_importance, min_opacity_thr, budget, opt, iteration, current_mask, camera=viewpoint_cam)
                
                if iteration % opt.opacity_reset_interval == 0:
                    gaussians.reset_opacity(0.05)

                if False and iteration % opt.opacity_reset_interval == 300 and iteration < 9000:
                    pass  # [no-prune] disabled per request
